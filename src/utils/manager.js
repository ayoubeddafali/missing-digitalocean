...
CrontabManager.prototype.add = function(key, tab, task, options, runOnce = false ) {
	...
	if ((typeof tab === 'string' || tab instanceof Date) && typeof key === 'string' && task instanceof Function) {
		...
		try {
			...
			if ( fooBaredByUser.name == "JobExist") { 
				throw {
					name: fooBaredByUser.name, 
					message: fooBaredByUser.message
				}
			} else {
					throw {
						name: "Error", 
						message: `Crontab: "${tab}" is probably Invalid, ${fooBaredByUser.message}`
					}
			}
		}
	}
		...
		throw { 
			name:        "Error", 
			message:     `couldn't add job, improper arguments`
		}
	}
}
...
CrontabManager.prototype.status = function(key) {
	return this.jobs[key].running
}
